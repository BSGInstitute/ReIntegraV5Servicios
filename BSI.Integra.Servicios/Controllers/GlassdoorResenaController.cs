...
{
    /// Controlador: GlassdoorResenaController
    /// Autor: Max Mantilla.
    /// Fecha: 21/04/2026
    /// <summary>
    /// Gestión de reseñas de empleador en Glassdoor para BSG Institute.
    ...
    /// Flujo: Controller → GlassdoorResenaService → GlassdoorResenaRepository → SP / EF Core.
    /// </summary>
    [Route("api/[controller]")]
    [ApiController]
    [EnableCors("CorsVista")]
    public class GlassdoorResenaController : ControllerBase
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ITokenManager _tokenManager;

        public GlassdoorResenaController(IUnitOfWork unitOfWork, ITokenManager tokenManager)
        {
        ...
        private IGlassdoorResenaService CrearServicio()
            => new GlassdoorResenaService(_unitOfWork);
        ...
        /// <param name="entidad">Entidad GlassdoorResena a insertar.</param>
        /// <returns>Entidad insertada.</returns>
        [HttpPost("Insertar")]
        public IActionResult Insertar([FromBody] GlassdoorResena entidad)
        {
        ...
        /// <param name="listado">Lista de entidades GlassdoorResena a insertar.</param>
        /// <returns>Lista de entidades insertadas.</returns>
        [HttpPost("InsertarLista")]
        public IActionResult InsertarLista([FromBody] List<GlassdoorResena> listado)
        {
            var claimsIdentity = User.Identity as ClaimsIdentity;
            var _respuestaCorrecta = ValidacionClaim.ValidarClaimFechaExpiracion(claimsIdentity);
            if (_respuestaCorrecta.TokenValida)
        ...
        /// <param name="entidad">Entidad GlassdoorResena con los datos actualizados.</param>
        /// <returns>Entidad actualizada.</returns>
        [HttpPut("Actualizar")]
        public IActionResult Actualizar([FromBody] GlassdoorResena entidad)
        {
            var claimsIdentity = User.Identity as ClaimsIdentity;
            var _respuestaCorrecta = ValidacionClaim.ValidarClaimFechaExpiracion(claimsIdentity);
            if (_respuestaCorrecta.TokenValida)
        ...
        /// <param name="listado">Lista de entidades GlassdoorResena a actualizar.</param>
        /// <returns>Lista de entidades actualizadas.</returns>
        [HttpPut("ActualizarLista")]
        public IActionResult ActualizarLista([FromBody] List<GlassdoorResena> listado)
        {
            var claimsIdentity = User.Identity as ClaimsIdentity;
            var _respuestaCorrecta = ValidacionClaim.ValidarClaimFechaExpiracion(claimsIdentity);
            if (_respuestaCorrecta.TokenValida)
        ...
        /// Ejecuta mkt.SP_GlassdoorResenaObtenerDatos.
        /// </summary>
        /// <param name="filtro">Filtros: visibilidad, país, tipo empleado, rango de fechas, paginación.</param>
        /// <returns>GlassdoorResenaGrillaPaginadaDTO con datos y metadatos de paginación.</returns>
        [HttpPost("[action]")]
        public IActionResult ObtenerGrilla([FromBody] GlassdoorResenaGrillaFiltroDTO filtro)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
        ...
        /// <returns>Lista de GlassdoorResenaPaisComboDTO con bandera.</returns>
        [HttpGet("[action]")]
        ...
        /// <returns>Lista de GlassdoorResenaCiudadComboDTO.</returns>
        ...
        [HttpPost("[action]")]
        public IActionResult MarcarResenaVisible([FromBody] GlassdoorResenaMarcarMostrarDTO dto)
        {
            var claimsIdentity = User.Identity as ClaimsIdentity;
            var _respuestaCorrecta = ValidacionClaim.ValidarClaimFechaExpiracion(claimsIdentity);
            if (_respuestaCorrecta.TokenValida)
        ...
        /// <returns>true si se actualizaron correctamente.</returns>
        [HttpPost("[action]")]
        public IActionResult MarcarResenaOculta([FromBody] GlassdoorResenaMarcarMostrarDTO dto)
        {
            var claimsIdentity = User.Identity as ClaimsIdentity;
            var _respuestaCorrecta = ValidacionClaim.ValidarClaimFechaExpiracion(claimsIdentity);
            if (_respuestaCorrecta.TokenValida)
