...
{
    public class EsquemaInterpretarInformacionService : IEsquemaInterpretarInformacionService
    {
        private readonly IEsquemaInterpretarInformacionRepository _repository;

        public EsquemaInterpretarInformacionService(IEsquemaInterpretarInformacionRepository repository)
        {
        ...
        public async Task<int> InsertarAsync(EsquemaInterpretarInformacionRequestDTO entidad, string usuario)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(entidad.Nombre)) throw new ArgumentException("El nombre de la interpretación es requerido");
        ...
        public async Task<int> ActualizarAsync(EsquemaInterpretarInformacionRequestDTO entidad, string usuario)
        {
            ...
            {
                if (entidad == null) throw new ArgumentNullException(nameof(entidad));
                if (entidad.Id == null || entidad.Id <= 0) throw new ArgumentException("El Id de la interpretación es requerido");
                if (string.IsNullOrWhiteSpace(entidad.Nombre)) throw new ArgumentException("El nombre de la interpretación es requerido");
                if (string.IsNullOrWhiteSpace(usuario)) throw new ArgumentException("El usuario es requerido");
        ...
        public async Task<int> EliminarAsync(int id, string usuario)
        {
            try
            {
                if (id <= 0) throw new ArgumentException("El Id de la interpretación es requerido");
                if (string.IsNullOrWhiteSpace(usuario)) throw new ArgumentException("El usuario es requerido");
