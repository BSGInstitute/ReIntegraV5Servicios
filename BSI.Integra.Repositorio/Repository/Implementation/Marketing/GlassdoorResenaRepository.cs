...
{
    /// Repositorio: GlassdoorResenaRepository
    /// Autor: Max Mantilla.
    /// Fecha: 21/04/2026
    /// <summary>
    /// Acceso a datos de la tabla mkt.T_GlassdoorResena.
    ...
    /// </summary>
    public class GlassdoorResenaRepository : GenericRepository<TGlassdoorResena>, IGlassdoorResenaRepository
    {
        private readonly Mapper _mapper;
        private readonly IntegraDBContext _integraContext;

        private const string SP_OBTENER_DATOS = "mkt.SP_GlassdoorResenaObtenerDatos";
        private const int TAMANO_PAGINA_DEFAULT = 50;
        private const int TAMANO_PAGINA_TODOS = 10000;

        public GlassdoorResenaRepository(
            ...
            : base(context, connectionFactory, dapperRepository)
        {
            ...
            var config = new MapperConfiguration(cfg =>
            {
                ...
                cfg.CreateMap<GlassdoorResena, GlassdoorResenaDTO>(MemberList.None).ReverseMap();
                cfg.CreateMap<GlassdoorResena, TGlassdoorResena>(MemberList.None).ReverseMap();
            });
        ...
        private TGlassdoorResena MapearAEntidad(GlassdoorResena entidad)
            => _mapper.Map<TGlassdoorResena>(entidad);
        ...
        /// <returns>TGlassdoorResena</returns>
        public TGlassdoorResena Add(GlassdoorResena entidad)
        {
        ...
        /// <returns>TGlassdoorResena</returns>
        public TGlassdoorResena Update(GlassdoorResena entidad)
        {
            var modelo = MapearAEntidad(entidad);
        ...
        /// <returns>IEnumerable de TGlassdoorResena</returns>
        public IEnumerable<TGlassdoorResena> Add(IEnumerable<GlassdoorResena> listadoEntidad)
        {
            var ahora = DateTime.UtcNow.AddHours(-5);
        ...
        /// <returns>IEnumerable de TGlassdoorResena</returns>
        public IEnumerable<TGlassdoorResena> Update(IEnumerable<GlassdoorResena> listadoEntidad)
        {
        ...
        public GlassdoorResenaGrillaPaginadaDTO ObtenerGrilla(GlassdoorResenaGrillaFiltroDTO filtro)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(resultado))
                    return new GlassdoorResenaGrillaPaginadaDTO { Pagina = numeroPagina, TamanoPagina = tamanoPagina };

                var items = JsonConvert.DeserializeObject<List<GlassdoorResenaGrillaItemConTotalDTO>>(resultado)
                            ?? new List<GlassdoorResenaGrillaItemConTotalDTO>();

                var totalRegistros = items.Count > 0 ? items[0].TotalRegistros : 0;

                return new GlassdoorResenaGrillaPaginadaDTO
                {
                    Data = items.Select(i => new GlassdoorResenaGrillaItemDTO
                    {
                        IdGlassdoorResena        = i.IdGlassdoorResena,
                        IdGlassdoorConfiguracion = i.IdGlassdoorConfiguracion,
                        Titulo                   = i.Titulo,
            ...
            {
                throw new Exception($"[GlassdoorResenaRepository.ObtenerGrilla] {ex.Message}", ex);
            }
        }
        ...
        public List<GlassdoorResenaPaisComboDTO> ObtenerPaisesCombo()
        {
            try
            {
                return _integraContext.TPais
                    ...
                    .Select(p => new GlassdoorResenaPaisComboDTO
                    {
                        IdPais = p.Id,
            ...
            {
                throw new Exception($"[GlassdoorResenaRepository.ObtenerPaisesCombo] {ex.Message}", ex);
            }
        }
        ...
        public List<GlassdoorResenaCiudadComboDTO> ObtenerCiudadesCombo(int idPais)
        {
            try
            {
                return _integraContext.TCiudads
                    ...
                    .OrderBy(c => c.Nombre)
                    .Select(c => new GlassdoorResenaCiudadComboDTO
                    {
                        IdCiudad = c.Id,
            ...
            {
                throw new Exception($"[GlassdoorResenaRepository.ObtenerCiudadesCombo] {ex.Message}", ex);
            }
        }
        ...
        public void MarcarResenaVisible(List<int> ids, string usuario)
        {
            ...
            {
                throw new Exception($"[GlassdoorResenaRepository.MarcarResenaVisible] {ex.Message}", ex);
            }
        }
        ...
        public void MarcarResenaOculta(List<int> ids, string usuario)
        {
            ...
            {
                throw new Exception($"[GlassdoorResenaRepository.MarcarResenaOculta] {ex.Message}", ex);
            }
        }
        ...
        private class GlassdoorResenaGrillaItemConTotalDTO : GlassdoorResenaGrillaItemDTO
        {
