using System;
using System.Collections.Generic;

namespace BSI.Integra.Persistencia.Modelos.IntegraDB
{
    public partial class TPaqueteTutorVirtual
    {
        public TPaqueteTutorVirtual()
        {
            TPaqueteTutorVirtualPais = new HashSet<TPaqueteTutorVirtualPai>();
        }

        /// <summary>
        /// Identificador autoincremental del paquete
        /// </summary>
        public int Id { get; set; }
        /// <summary>
        /// Nombre del paquete de tutor virtual
        /// </summary>
        public string Nombre { get; set; } = null!;
        /// <summary>
        /// Cantidad de créditos incluidos en el paquete
        /// </summary>
        public int CantidadCredito { get; set; }
        /// <summary>
        /// Estado lógico del registro (1=Activo, 0=Inactivo)
        /// </summary>
        public bool Estado { get; set; }
        /// <summary>
        /// Usuario que creó el registro
        /// </summary>
        public string UsuarioCreacion { get; set; } = null!;
        /// <summary>
        /// Usuario que modificó el registro por última vez
        /// </summary>
        public string UsuarioModificacion { get; set; } = null!;
        /// <summary>
        /// Fecha de creación del registro
        /// </summary>
        public DateTime FechaCreacion { get; set; }
        /// <summary>
        /// Fecha de última modificación del registro
        /// </summary>
        public DateTime FechaModificacion { get; set; }
        /// <summary>
        /// Control de concurrencia (timestamp)
        /// </summary>
        public byte[] RowVersion { get; set; } = null!;

        public virtual ICollection<TPaqueteTutorVirtualPai> TPaqueteTutorVirtualPais { get; set; }
    }
}
